...
public:
    ...
    {
        return GlobalPoint(v[0] - o[0], v[1] - o[1], v[2] - o[2]);
    }
    ...
    GlobalPoint operator*(const float scalar) const
    {
...
inline float perp2(const GlobalPoint &p)
{
    return p[0] * p[0] + p[1] * p[1];
}

inline float length2(const GlobalPoint &a)
{
    return dot(a, a);
}

inline float length(const GlobalPoint &a)
{
...
inline float dot(const GlobalPoint &a, const GlobalPoint &b)
{
